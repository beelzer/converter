...
test.describe("Audio/video toolkit — real fixtures", () => {
  ...
  test("Frames mode extracts a single PNG frame from the MP4 fixture", async ({
    ...
    // PNG IHDR sits at byte 16; verify the extracted frame keeps source dims.
    ...
    report(testInfo, {
      input: { path: FIXTURES.av.clipMp4, label: "clip.mp4 — 1280×720 HD" },
      output: { path: outPath, label: `Extracted frame at ${width}×${height}` },
      notes: "Frame should preserve the source 720p dimensions — no implicit downscaling.",
    });
  });
