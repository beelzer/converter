...
This site (convert.dcln.me) is a collection of free file conversion tools that run entirely in the user's browser. Files are not uploaded to any server.
...
- Vulnerabilities in the in-browser conversion pipeline (`src/lib/`, Web Workers, file handling) that could read files the user did not explicitly choose
...
- **Asset CDN**: Cloudflare R2 bucket exposed at `cdn.dcln.me`, CORS scoped to `https://convert.dcln.me`
- **Security headers**: `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `X-Frame-Options` enforced via `public/_headers`
- **Conversion isolation**: All file processing runs in Web Workers in the user's browser — no server-side code path touches user files
